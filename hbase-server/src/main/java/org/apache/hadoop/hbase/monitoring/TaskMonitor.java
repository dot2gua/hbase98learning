...
public class TaskMonitor {
    ...
    public synchronized MonitoredTask createStatus(String description) {
        ...
        MonitoredTask proxy = (MonitoredTask) Proxy.newProxyInstance(stat.getClass().getClassLoader(), new Class<?>[] {
                MonitoredTask.class }, new PassthroughInvocationHandler<MonitoredTask>(stat));
        TaskAndWeakRefPair pair = new TaskAndWeakRefPair(stat,
                                                         proxy);
        if (tasks.isFull()) {
            purgeExpiredTasks();
        }
    ...
    public synchronized MonitoredRPCHandler createRPCStatus(String description) {
        ...
        stat.setDescription(description);
        MonitoredRPCHandler proxy = (MonitoredRPCHandler) Proxy.newProxyInstance(stat.getClass().getClassLoader(), new Class<?>[] {
                MonitoredRPCHandler.class }, new PassthroughInvocationHandler<MonitoredRPCHandler>(stat));
        TaskAndWeakRefPair pair = new TaskAndWeakRefPair(stat,
                                                         proxy);
        tasks.add(pair);
        return proxy;
     ...
     * passes through calls to that object. In art form: <code> Proxy
     ...
     * means that they can leak that object, and we'll detect it since our weak
     * reference will go null. But, we still have the actual object, so we can log
     * it and display it as a leaked (incomplete) action.
     */
