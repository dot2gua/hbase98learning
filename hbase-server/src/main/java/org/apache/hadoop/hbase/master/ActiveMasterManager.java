...
public class ActiveMasterManager extends ZooKeeperListener {
     ...
     * from a nodeCreated or nodeDeleted event because there are no guarantees that
     * the current state of the master node matches the event at the time of our
     * next ZK request.
     *
     * <p>
     * Uses the watchAndCheckExists method which watches the master address node
     * regardless of whether it exists or not. If it does exist (there is an active
     * master), it returns true. Otherwise it returns false.
     *
     * <p>
     ...
     *         was running or if some other problem (zookeeper, stop flag has been
     *         set on this Master)
     */
        ...
        while (true) {
            ...
            try {
                ...
                if (MasterAddressTracker.setMasterAddress(this.watcher, this.watcher.getMasterAddressZNode(), this.sn)) {
                    ...
                    // Save the znode in a file, this will allow to check if we crash in the launch
                    // scripts
                ...
                /*
                 * Add a ZNode for ourselves in the backup master directory since we are not the
                 * active master.
                 *
                 * If we become the active master later, ActiveMasterManager will delete this
                 * node explicitly. If we crash before then, ZooKeeper will delete this node for
                 * us since it is ephemeral.
                 */
                ...
                if (bytes == null) {
                    ...
                    if (ServerName.isSameHostnameAndPort(currentMaster, this.sn)) {
                        ...
                        // we delete the file anyway: a second attempt to delete the znode is likely to
                        // fail again.
