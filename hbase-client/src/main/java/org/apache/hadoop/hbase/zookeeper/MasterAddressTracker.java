 ...
 * <code>/master</code> will contain the address of the current master. This
 * listener is interested in <code>NodeDeleted</code> and
 * <code>NodeCreated</code> events on <code>/master</code>.
 * <p>
...
public class MasterAddressTracker extends ZooKeeperNodeTracker {
    /**
     * Construct a master address listener with the specified <code>zookeeper</code>
     * reference.
     * <p>
     * This constructor does not trigger any actions, you must call methods
     * explicitly. Normally you will just want to execute {@link #start()} to begin
     * tracking of the master address.
     *
     * @param watcher
    ...
    /**
     * Get the address of the current master if one is available. Returns null if no
     * current master.
     * 
    ...
    /**
     * Get the address of the current master if one is available. Returns null if no
     * current master. If refresh is set, try to load the data from ZK again,
    ...
    /**
     * Get master address. Use this instead of {@link #getMasterAddress()} if you do
     * not have an instance of this tracker in your context.
     * 
     * @param zkw
     *            ZooKeeperWatcher to use
     * @return ServerName stored in the the master address znode or null if no znode
     *         present.
     * @throws KeeperException
     * @throws IOException
     */
    public static ServerName getMasterAddress(final ZooKeeperWatcher zkw) throws KeeperException, IOException {
     ...
     * @param znode
     *            Where to create the znode; could be at the top level or it could
     *            be under backup masters
     * @param master
     ...
     * @param sn
     *            must not be null
     * @return Content of the master znode as a serialized pb with the pb magic as
     *         prefix.
     */
