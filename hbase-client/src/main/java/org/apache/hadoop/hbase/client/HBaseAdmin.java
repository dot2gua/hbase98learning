 ...
 * <p>
 * Currently HBaseAdmin instances are not expected to be long-lived. For
...
public class HBaseAdmin implements Abortable, Closeable {
    ...
    /**
     * Constructor for externally managed HConnections. The connection to master
     * will be created when required by admin functions.
     *
     * @param connection
     *            The HConnection instance to use
     * @throws MasterNotRunningException,
     *             ZooKeeperConnectionException are not thrown anymore but kept into
     *             the interface for backward api compatibility
     */
    ...
    /**
     * @return A new CatalogTracker instance; call
     *         {@link #cleanupCatalogTracker(CatalogTracker)} to cleanup the
     *         returned catalog tracker.
    ...
    synchronized CatalogTracker getCatalogTracker()
        ...
        try {
            ...
            // If we did not succeed but created a catalogtracker, clean it up. CT has a ZK
            // instance
     ...
     * only exists as part of the hbase:meta table's region info.
     *
    ...
    /**
     * List all of the names of userspace tables matching the given regular
     * expression.
     * 
     * @param pattern
     *            The regular expression to match against
     ...
     * specified will become the end key of the first region of the table, and the
     * end key specified will become the start key of the last region of the table
     * (the first region has a null start key and the last region has a null end
     * key).
     *
     ...
     * of split keys plus one. Synchronous operation. Note : Avoid passing empty
     * split key.
     *
     * @param desc
     *            table descriptor for table
            ...
            if (!doneWithMetaScan) {
                ...
                if (actualRegCount.get() < numRegs) {
                    if (tries == this.numRetries * this.retryLongerMultiplier - 1) {
                        throw new RegionOfflineException("Only " + actualRegCount.get() + " of "
                                + numRegs + " regions are online; retries exhausted.");
                    }
                    try { // Sleep
                        Thread.sleep(getPauseTime(tries));
                    } catch (InterruptedException e) {
                        throw new InterruptedIOException("Interrupted when opening" + " regions; "
                                + actualRegCount.get() + " of " + numRegs
     ...
     * {@link #isTableAvailable} -- it is not safe to create an HTable instance to
     * this table before it is available. Note : Avoid passing empty split key.
     * 
     * @param desc
     *            table descriptor for table
     *
    ...
    public void createTableAsync(final HTableDescriptor desc,
        ...
        executeCallable(new MasterCallable<Void>(getConnection()) {
            ...
            public Void call() throws ServiceException {
                CreateTableRequest request = RequestConverter.buildCreateTableRequest(desc,
                                                                                      splitKeys);
    ...
    /**
     * Wait for the table to be enabled and available If enabling the table exceeds
     * the retry period, an exception is thrown.
     * 
     * @param tableName
     *            name of the table
     * @throws IOException
     *             if a remote or network exception occurs or table is not enabled
     *             after the retries period.
     */
    private void waitUntilTableIsEnabled(final TableName tableName) throws IOException {
        ...
        if (!enabled) {
            ...
            throw new IOException("Table '" + tableName + "' not yet enabled, after " + msec
                    + "ms.");
        }
    }

    /**
     * Brings a table on-line (enables it). Method returns immediately though enable
     * of table may take some time to complete, especially if the table is large
     * (All regions are opened as part of enabling process). Check
     * {@link #isTableEnabled(byte[])} to learn when table is fully online. If table
     * is taking too long to online, check server logs.
     * 
     * @param tableName
     * @throws IOException
     * @since 0.90.0
     ...
     * the servers to stop serving it. This method returns immediately. The disable
     * of a table can take some time if the table is large (all regions are closed
     * as part of table disable operation). Call {@link #isTableDisabled(byte[])} to
     * check for when disable completes. If table is taking too long to online,
     * check server logs.
     * 
     * @param tableName
     *            name of table
    ...
    /**
     * Use this api to check if the table has been created with the specified number
     * of splitkeys which was used while creating the given table. Note : If this
     * api is used after a table's region gets splitted, the api may return false.
     * 
     * @param tableName
     *            name of table to check
     * @param splitKeys
     ...
     *         regions that are yet to be updated Pair.getSecond() is the total
    ...
    public Pair<Integer, Integer> getAlterStatus(final TableName tableName) throws IOException {
        return executeCallable(new MasterCallable<Pair<Integer, Integer>>(getConnection()) {
            ...
            public Pair<Integer, Integer> call() throws ServiceException {
                ...
                Pair<Integer, Integer> pair = new Pair<Integer, Integer>(Integer.valueOf(ret.getYetToUpdateRegions()),
                                                                         Integer.valueOf(ret.getTotalRegions()));
                return pair;
            }
        });
    }
     ...
     *             if a remote or network exception occurs
     */
    public void addColumn(final TableName tableName,
     ...
     *             if a remote or network exception occurs
     */
    public void deleteColumn(final String tableName,
                             final String columnName)
            throws IOException {
        deleteColumn(TableName.valueOf(tableName), Bytes.toBytes(columnName));
    }
     ...
     *             if a remote or network exception occurs
     */
    public void deleteColumn(final TableName tableName,
                             final byte[] columnName)
            throws IOException {
        executeCallable(new MasterCallable<Void>(getConnection()) {
            @Override
            public Void call() throws ServiceException {
                DeleteColumnRequest req = RequestConverter.buildDeleteColumnRequest(tableName,
                                                                                    columnName);
     ...
     *             if a remote or network exception occurs
     */
    public void modifyColumn(final byte[] tableName,
                             HColumnDescriptor descriptor)
            throws IOException {
        modifyColumn(TableName.valueOf(tableName), descriptor);
    }
     ...
     *             if a remote or network exception occurs
     */
    public void modifyColumn(final TableName tableName,
                             final HColumnDescriptor descriptor)
            throws IOException {
        executeCallable(new MasterCallable<Void>(getConnection()) {
            @Override
            public Void call() throws ServiceException {
                ModifyColumnRequest req = RequestConverter.buildModifyColumnRequest(tableName,
                                                                                    descriptor);
    ...
    /**
     * Close a region. For expert-admins. Runs close on the regionserver. The master
     * will not be informed of the close.
     * 
     * @param regionname
     *            region name to close
     * @param serverName
     ...
     *             if a remote or network exception occurs
     */
    public void closeRegion(final String regionname,
     ...
     *            region name to close
     * @param serverName
     *            The servername of the regionserver. If passed null we will use
     *            servername found in the hbase:meta table. A server name is made of
     *            host, port and startcode. Here is an example:
     *            <code> host187.example.com,60020,1289493121758</code>
     * @throws IOException
     *             if a remote or network exception occurs
     */
    public void closeRegion(final byte[] regionname,
                            final String serverName)
            throws IOException {
        CatalogTracker ct = getCatalogTracker();
        try {
    ...
    public boolean closeRegionWithEncodedRegionName(final String encodedRegionName,
        ...
        CloseRegionRequest request = RequestConverter.buildCloseRegionRequest(sn,
                                                                              encodedRegionName,
                                                                              false);
        try {
     ...
     * @throws IOException
     */
    public void closeRegion(final ServerName sn,
    ...
    private void flush(final ServerName sn,
                       final HRegionInfo hri)
            throws IOException {
        AdminService.BlockingInterface admin = this.connection.getAdmin(sn);
    ...
    /**
     * Major compact a column family within a table or region. Asynchronous
     * operation.
     *
     * @param tableNameOrRegionName
     *            table or region to major compact
     ...
     *            column family within a table or region
     * @param major
     *            True if we are to do a major compaction.
     * @throws IOException
     *             if a remote or network exception occurs
     * @throws InterruptedException
     */
        ...
        try {
            ...
            if (regionServerPair != null) {
                if (regionServerPair.getSecond() == null) {
                    ...
                    compact(regionServerPair.getSecond(),
                            regionServerPair.getFirst(),
                            major,
                            columnFamily);
                }
            } else {
                    ...
                    try {
                        ...
                        if (LOG.isDebugEnabled()) {
                            LOG.debug("Trying to" + (major ? " major" : "") + " compact "
                                    + pair.getFirst() + ": " + StringUtils.stringifyException(e));
                        }
                    }
                }
            }
    ...
    private void compact(final ServerName sn,
                         final HRegionInfo hri,
                         final boolean major,
                         final byte[] family)
            throws IOException {
        AdminService.BlockingInterface admin = this.connection.getAdmin(sn);
        CompactRegionRequest request = RequestConverter.buildCompactRegionRequest(hri.getRegionName(),
                                                                                  major,
                                                                                  family);
        try {
     ...
     * @param major
     *            if it's major compaction
     * @throws IOException
     * @throws InterruptedException
     */
    public void compactRegionServer(final ServerName sn,
                                    boolean major)
            throws IOException, InterruptedException {
     ...
     *            The servername of the destination regionserver. If passed the
     ...
     * assigned to a regionserver chosen at random. Region could be reassigned back
     * to the same server. Use {@link #move(byte[], byte[])} if you want to control
     * the region movement.
     * 
     * @param regionName
     *            Region to unassign. Will clear any existing RegionPlan if one
     *            found.
     * @param force
     *            If true, force unassign (Will remove region from
     *            regions-in-transition too if present. If results in double
     *            assignment use hbck -fix to resolve. To be used by experts).
     * @throws MasterNotRunningException
     * @throws ZooKeeperConnectionException
     * @throws IOException
     */
    public void unassign(final byte[] regionName,
                         final boolean force)
            throws MasterNotRunningException, ZooKeeperConnectionException, IOException {
    ...
    /**
     * Offline specified region from master's in-memory state. It will not attempt
     * to reassign the region as in unassign. This API can be used when a region not
     * served by any region server and still online as per Master's in memory state.
     * If this API is incorrectly used on active region then master will loose track
     * of that region.
     *
    ...
    public boolean setBalancerRunning(final boolean on,
        ...
        MasterKeepAliveConnection stub = connection.getKeepAliveMasterService();
        try {
            SetBalancerRunningRequest req = RequestConverter.buildSetBalancerRunningRequest(on,
                                                                                            synchronous);
            ...
            // Throwing MasterNotRunningException even though not really valid in order to
            // not
            // break interface by adding additional exception type.
            throw new MasterNotRunningException("Unexpected exception when calling balanceSwitch",
                                                se);
        } finally {
            stub.close();
        }
    ...
    /**
     * Invoke the balancer. Will run the balancer and if regions to move, it will go
     * ahead and do the reassignments. Can NOT run for various reasons. Check logs.
     * 
    ...
    /**
     * Query the state of the balancer from the Master. It's not a guarantee that
     * the balancer is actually running this very moment, but that it will run.
     *
     * @return True if the balancer is enabled, false otherwise.
     */
    public boolean isBalancerEnabled() throws ServiceException, MasterNotRunningException {
        MasterKeepAliveConnection stub = connection.getKeepAliveMasterService();
        try {
     ...
     * 
     * @param enable
     *            if true enables the catalog janitor
    ...
    public int runCatalogScan() throws ServiceException, MasterNotRunningException {
        MasterKeepAliveConnection stub = connection.getKeepAliveMasterService();
        try {
            return stub.runCatalogScan(null, RequestConverter.buildCatalogScanRequest())
                       .getScanResult();
        } finally {
            stub.close();
        }
     ...
     * 
     * @param encodedNameOfRegionA
     *            encoded name of region a
     * @param encodedNameOfRegionB
     *            encoded name of region b
     * @param forcible
    ...
    public void mergeRegions(final byte[] encodedNameOfRegionA,
        ...
        MasterKeepAliveConnection master = connection.getKeepAliveMasterService();
        try {
            DispatchMergingRegionsRequest request = RequestConverter.buildDispatchMergingRegionsRequest(encodedNameOfRegionA,
    ...
    private void split(final ServerName sn,
                       final HRegionInfo hri,
                       byte[] splitPoint)
            throws IOException {
     ...
     * This means that it may be a while before your schema change is updated across
     * all of the table.
     *
     * @param tableName
     *            name of table.
     ...
     *             if a remote or network exception occurs
     */
    public void modifyTable(final TableName tableName,
    ...
    public void modifyTable(final byte[] tableName,
                            final HTableDescriptor htd)
            throws IOException {
        modifyTable(TableName.valueOf(tableName), htd);
    }

    public void modifyTable(final String tableName,
                            final HTableDescriptor htd)
            throws IOException {
        modifyTable(TableName.valueOf(tableName), htd);
    }
    ...
    /**
     * @param tableNameOrRegionName
     *            Name of a table or name of a region.
     * @param ct
     *            A {@link CatalogTracker} instance (caller of this method usually
     *            has one).
     * @return a pair of HRegionInfo and ServerName if
     *         <code>tableNameOrRegionName</code> is a verified region name (we call
     *         {@link MetaReader#getRegion( CatalogTracker, byte[])} else null.
     ...
     * region name, the corresponding region is found from meta and its region name
     * is returned. If we can't find any region in meta matching the input as either
     * region name or encoded region name, the input is returned as is. We don't
     * throw unknown region exception.
     */
    ...
    public synchronized void stopRegionServer(final String hostnamePort) throws IOException {
        ...
        AdminService.BlockingInterface admin = this.connection.getAdmin(ServerName.valueOf(hostname,
                                                                                           port,
                                                                                           0));
        StopServerRequest request = RequestConverter.buildStopServerRequest("Called by admin client "
        ...
        return executeCallable(new MasterCallable<IsInMaintenanceModeResponse>(getConnection()) {
            ...
            public IsInMaintenanceModeResponse call() throws ServiceException {
                return master.isMasterInMaintenanceMode(null,
                                                        IsInMaintenanceModeRequest.newBuilder()
                                                                                  .build());
            }
    ...
    private HRegionLocation getFirstMetaServerForTable(final TableName tableName)
        ...
        return connection.locateRegion(TableName.META_TABLE_NAME,
                                       HRegionInfo.createRegionName(tableName,
                                                                    null,
                                                                    HConstants.NINES,
                                                                    false));
    }
            ...
            public Void call() throws Exception {
                master.createNamespace(null,
                                       CreateNamespaceRequest.newBuilder()
                                                             .setNamespaceDescriptor(ProtobufUtil.toProtoNamespaceDescriptor(descriptor))
                                                             .build());
                return null;
            }
        });
    ...
    public NamespaceDescriptor getNamespaceDescriptor(final String name) throws IOException {
        return executeCallable(new MasterCallable<NamespaceDescriptor>(getConnection()) {
            ...
            public NamespaceDescriptor call() throws Exception {
                return ProtobufUtil.toNamespaceDescriptor(master.getNamespaceDescriptor(null,
    ...
    public NamespaceDescriptor[] listNamespaceDescriptors() throws IOException {
        return executeCallable(new MasterCallable<NamespaceDescriptor[]>(getConnection()) {
            @Override
            public NamespaceDescriptor[] call() throws Exception {
                List<HBaseProtos.NamespaceDescriptor> list = master.listNamespaceDescriptors(null,
                                                                                             ListNamespaceDescriptorsRequest.newBuilder()
                                                                                                                            .build())
    ...
    public HTableDescriptor[] listTableDescriptorsByNamespace(final String name)
        ...
        return executeCallable(new MasterCallable<HTableDescriptor[]>(getConnection()) {
            ...
            public HTableDescriptor[] call() throws Exception {
                List<TableSchema> list = master.listTableDescriptorsByNamespace(null,
    ...
    public TableName[] listTableNamesByNamespace(final String name) throws IOException {
        return executeCallable(new MasterCallable<TableName[]>(getConnection()) {
            ...
            public TableName[] call() throws Exception {
                List<HBaseProtos.TableName> tableNames = master.listTableNamesByNamespace(null,
                                                                                          ListTableNamesByNamespaceRequest.newBuilder()
                                                                                                                          .setNamespaceName(name)
                                                                                                                          .build())
                                                               .getTableNameList();
    ...
    /**
     * Check to see if HBase is running. Throw an exception if not. We consider that
     * HBase is running if ZooKeeper and Master are running.
     *
     * @param conf
    ...
    /**
     * Get the current compaction state of a table or region. It could be in a major
     * compaction, a minor compaction, both, or none.
     *
     * @param tableNameOrRegionName
     *            table or region to major compact
     * @throws IOException
        ...
        try {
            ...
            if (regionServerPair != null) {
                if (regionServerPair.getSecond() == null) {
                    ...
                    AdminService.BlockingInterface admin = this.connection.getAdmin(sn);
                    GetRegionInfoRequest request = RequestConverter.buildGetRegionInfoRequest(regionServerPair.getFirst()
                                                                                                              .getRegionName(),
                                                                                              true);
                ...
                for (Pair<HRegionInfo, ServerName> pair : pairs) {
                    ...
                    try {
                        ...
                        AdminService.BlockingInterface admin = this.connection.getAdmin(sn);
                        GetRegionInfoRequest request = RequestConverter.buildGetRegionInfoRequest(pair.getFirst()
                                                                                                      .getRegionName(),
                                                                                                  true);
                        GetRegionInfoResponse response = admin.getRegionInfo(null, request);
                        ...
                        if (e.getMessage()
                             .indexOf(NotServingRegionException.class.getName()) >= 0) {
                            if (LOG.isDebugEnabled()) {
                                LOG.debug("Trying to get compaction state of " + pair.getFirst()
                                        + ": " + StringUtils.stringifyException(e));
                            }
    ...
    /**
     * Take a snapshot for the given table. If the table is enabled, a FLUSH-type
     * snapshot will be taken. If the table is disabled, an offline snapshot is
     * taken.
     * <p>
     * Snapshots are considered unique based on <b>the name of the snapshot</b>.
     * Attempts to take a snapshot with the same name (even a different type or with
     ...
     *            name of the table for which snapshot is created
     * @param flushType
     *            if the snapshot should be taken without flush memstore first
     * @throws IOException
     *             if a remote or network exception occurs
     * @throws SnapshotCreationException
     *             if snapshot creation failed
    ...
    /**
     * public void snapshot(final String snapshotName, Create a timestamp consistent
     * snapshot for the given table. final byte[] tableName) throws IOException,
     * <p>
     * Snapshots are considered unique based on <b>the name of the snapshot</b>.
     * Attempts to take a snapshot with the same name (even a different type or with
     * different parameters) will fail with a {@link SnapshotCreationException}
     ...
     * 
     * @param snapshotName
     *            name to give the snapshot on the filesystem. Must be unique from
     *            all other snapshots stored on the cluster
     * @param tableName
     *            name of the table to snapshot
     * @param type
     *            type of snapshot to take
     * @throws IOException
     *             we fail to reach the master
     * @throws SnapshotCreationException
     *             if snapshot creation failed
     * @throws IllegalArgumentException
     *             if the snapshot request is formatted incorrectly
     ...
     * <p>
     * Only a single snapshot should be taken at a time for an instance of HBase, or
     * results may be undefined (you can tell multiple HBase clusters to snapshot at
     * the same time, but only one at a time for a single cluster).
     * <p>
     * Snapshots are considered unique based on <b>the name of the snapshot</b>.
     * Attempts to take a snapshot with the same name (even a different type or with
     * different parameters) will fail with a {@link SnapshotCreationException}
     ...
     * {@link org.apache.hadoop.hbase.TableName#isLegalFullyQualifiedTableName(byte[])}.
     * <p>
     * You should probably use {@link #snapshot(String, String)} or
     * {@link #snapshot(byte[], byte[])} unless you are sure about the type of
     * snapshot that you want to take.
     * 
     * @param snapshot
     *            snapshot to take
     * @throws IOException
     *             or we lose contact with the master.
     * @throws SnapshotCreationException
     *             if snapshot failed to be taken
     * @throws IllegalArgumentException
     *             if the snapshot request is formatted incorrectly
     */
    public void snapshot(SnapshotDescription snapshot)
        ...
        while (tries == 0 || ((EnvironmentEdgeManager.currentTimeMillis() - start) < max
                && !done.getDone())) {
            try {
    ...
    /**
     * Take a snapshot without waiting for the server to complete that snapshot
     * (asynchronous)
     * <p>
     * Only a single snapshot should be taken at a time, or results may be
     * undefined.
     * 
     * @param snapshot
     *            snapshot to take
     ...
     * <li>finished with error - throws the exception that caused the snapshot to
     * fail</li>
     * </ol>
     * <p>
     * The cluster only knows about the most recent snapshot. Therefore, if another
     * snapshot has been run/started since the snapshot your are checking, you will
     * recieve an {@link UnknownSnapshotException}.
     * 
     * @param snapshot
     *            description of the snapshot to check
     * @return <tt>true</tt> if the snapshot is completed, <tt>false</tt> if the
     *         snapshot is still running
     * @throws IOException
     *             if we have a network issue
     ...
     * disabled) If the "hbase.snapshot.restore.take.failsafe.snapshot"
     * configuration property is set to true, a snapshot of the current table is
     * taken before executing the restore operation. In case of restore failure, the
     * failsafe snapshot will be restored. If the restore completes without problem
     * the failsafe snapshot is deleted.
     *
     * @param snapshotName
    ...
    public void restoreSnapshot(final String snapshotName)
            throws IOException, RestoreSnapshotException {
        boolean takeFailSafeSnapshot = conf.getBoolean("hbase.snapshot.restore.take.failsafe.snapshot",
                                                       false);
    ...
    /**
     * Restore the specified snapshot on the original table. (The table must be
     * disabled) If 'takeFailSafeSnapshot' is set to true, a snapshot of the current
     * table is taken before executing the restore operation. In case of restore
     * failure, the failsafe snapshot will be restored. If the restore completes
     * without problem the failsafe snapshot is deleted.
     *
    ...
    public void restoreSnapshot(final String snapshotName,
        ...
        if (tableName == null) {
            throw new RestoreSnapshotException("Unable to find the table name for snapshot="
                    + snapshotName);
        }
        ...
        if (takeFailSafeSnapshot) {
            ...
            failSafeSnapshotSnapshotName = failSafeSnapshotSnapshotName.replace("{snapshot.name}",
                                                                                snapshotName)
                                                                       .replace("{table.name}",
                                                                                tableName.toString()
        ...
        try {
            ...
            if (takeFailSafeSnapshot) {
                try {
                    ...
                    String msg = "Failed to restore and rollback to snapshot="
     ...
     */
    public void cloneSnapshot(final byte[] snapshotName,
                              final byte[] tableName)
     ...
     *             if the specified table has not a valid name
     */
    public void cloneSnapshot(final String snapshotName,
                              final String tableName)
            throws IOException, TableExistsException, RestoreSnapshotException,
            InterruptedException {
     ...
     *
     * @param signature
     *            A distributed procedure is uniquely identified by its signature
     *            (default the root ZK node name of the procedure).
     * @param instance
     *            The instance name of the procedure. For some procedures, this
     ...
     * <li>running - returns <tt>false</tt></li>
     * <li>finished - returns <tt>true</tt></li>
     * <li>finished with error - throws the exception that caused the procedure to
     * fail</li>
     * </ol>
     * <p>
     *
     * @param signature
     *            The signature that uniquely identifies a procedure
     * @param instance
     *            The instance name of the procedure
     * @param props
     *            Property/Value pairs of properties passing to the procedure
     * @return true if the specified procedure is finished successfully, false if it
     *         is still running
     * @throws IOException
    ...
    public boolean isProcedureFinished(String signature,
        ...
        return executeCallable(new MasterCallable<IsProcedureDoneResponse>(getConnection()) {
            ...
            public IsProcedureDoneResponse call() throws ServiceException {
                return master.isProcedureDone(null,
                                              IsProcedureDoneRequest.newBuilder()
                                                                    .setProcedure(desc)
                                                                    .build());
            }
        }).getDone();
     ...
     * (blocking). To check if the cloned table exists, use
     * {@link #isTableAvailable} -- it is not safe to create an HTable instance to
     * this table before it is available.
     * 
     * @param snapshotName
     *            snapshot to restore
     * @param tableName
     *            table name to restore the snapshot on
     * @throws IOException
     *             if a remote or network exception occurs
     * @throws RestoreSnapshotException
     *             if snapshot failed to be restored
     ...
     *             if the restore request is formatted incorrectly
     */
    private void internalRestoreSnapshot(final String snapshotName,
                                         final TableName tableName)
            throws IOException, RestoreSnapshotException {
        ...
        while (!done.getDone()) {
            try {
                ...
                LOG.debug("Interrupted while waiting for snapshot " + snapshot
                        + " restore to complete");
                Thread.currentThread().interrupt();
            }
    ...
    public List<SnapshotDescription> listSnapshots() throws IOException {
        return executeCallable(new MasterCallable<List<SnapshotDescription>>(getConnection()) {
            ...
            public List<SnapshotDescription> call() throws ServiceException {
                return master.getCompletedSnapshots(null,
                                                    GetCompletedSnapshotsRequest.newBuilder()
                                                                                .build())
                             .getSnapshotsList();
            }
        });
     ...
     * expression and snapshot name regular expression.
     * 
     * @param tableNameRegex
     *            The table name regular expression to match against
     * @param snapshotNameRegex
     *            The snapshot name regular expression to match against
            ...
            public Void call() throws ServiceException {
                master.deleteSnapshot(null,
                                      DeleteSnapshotRequest.newBuilder()
                                                           .setSnapshot(SnapshotDescription.newBuilder()
                                                                                           .setName(snapshotName)
                                                                                           .build())
                                                           .build());
                return null;
    ...
    public void deleteTableSnapshots(Pattern tableNamePattern,
                                     Pattern snapshotNamePattern)
            throws IOException {
        List<SnapshotDescription> snapshots = listTableSnapshots(tableNamePattern,
                                                                 snapshotNamePattern);
        for (SnapshotDescription snapshot : snapshots) {
            try {
                internalDeleteSnapshot(snapshot);
    ...
    /**
     * Parent of {@link MasterCallable} and {@link MasterCallable}. Has common
     * methods.
     * 
     * @param <V>
     */
    abstract static class MasterCallable<V> implements RetryingCallable<V>, Closeable {
        ...
        public void close() throws IOException {
            // The above prepare could fail but this would still be called though
            // masterAdmin is null
            if (this.master != null)
                this.master.close();
        }
     ...
     *
     * <p>
     * The obtained {@link com.google.protobuf.RpcChannel} instance can be used to
     * access a published coprocessor {@link com.google.protobuf.Service} using
     * standard protobuf service invocations:
     * </p>
     *
     ...
     *
     * @param sn
     *            the server name to which the endpoint call is made
        ...
        try {
            return executeCallable(new MasterCallable<List<SecurityCapability>>(getConnection()) {
                ...
                public List<SecurityCapability> call() throws ServiceException {
                    SecurityCapabilitiesRequest req = SecurityCapabilitiesRequest.newBuilder()
                                                                                 .build();
                    return ProtobufUtil.toSecurityCapabilityList(master.getSecurityCapabilities(null,
                                                                                                req)
                                                                       .getCapabilitiesList());
                }
            });
